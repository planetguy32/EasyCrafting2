package me.planetguy.tct.inventory.gui;
...
import me.planetguy.tct.cruft.TileEntityEasyCrafting;
import me.planetguy.tct.inventory.tab.GuiTabbed;
import me.planetguy.tct.inventory.tab.Tab;
...
import me.planetguy.tct.util.InventoryUtils;
import me.planetguy.tct.util.ItemMap;
import me.planetguy.tct.util.PacketSender;
import me.planetguy.tct.util.StackUtils;
import me.planetguy.tct.util.WrappedStack;
...
public class GuiTCT extends GuiTabbed implements IContainerTooltipHandler {

	private class TabTCT extends Tab {
		public TabTCT(ItemStack iconStack, String tooltip) {
	...
	public GuiTCT(InventoryPlayer playerInventory, TileEntityEasyCrafting tileInventory) {
	...
	public void initTabs() {
		tabGroup.addTab(new TabTCT(new ItemStack(Blocks.crafting_table), "Crafting"));
		tabGroup.addTab(new TabTCT(new ItemStack(Blocks.golden_rail), "TurboCrafting"));
		tabGroup.addTab(new TabTCT(new ItemStack(Items.compass), "Search"));
	}
	...
	private void onCraftingSlotClick(Slot slot, int slotIndex, int button, int action) {
		...
		if (finalStackSize > 0) {
			...
			//EasyCrafting uses server-side stuff here, but a design goal is to be client-side-only -> must replace with network calls
			//PacketHandler.INSTANCE.sendToServer(new MessageEasyCrafting(recipe, isRightClick, isShiftClick));
			
			PacketSender.craftItem(recipe, isRightClick, isShiftClick);
